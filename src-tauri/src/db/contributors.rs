...
pub fn update_contributor_stats(
    ...
    conn.execute(
        r#"
        UPDATE contributors 
        SET commit_count = ?1, additions = ?2, deletions = ?3, 
            active_days = ?4, last_commit_at = ?5, updated_at = ?6
...
pub fn get_top_contributors(
    ...
    let mut sql = "SELECT id, repo_id, author_name, author_email, commit_count, 
                   additions, deletions, active_days, last_commit_at, updated_at 
            FROM contributors 
            WHERE repo_id = ?1 

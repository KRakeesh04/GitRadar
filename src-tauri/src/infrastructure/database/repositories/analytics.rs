// Analytics and activity tracking operations
use crate::infrastructure::database::models::contributor::Contributor;
use crate::infrastructure::database::models::RepoActivityDaily;
...
pub fn insert_repo_activity_daily(
    ...
    conn.execute(r#"INSERT OR REPLACE INTO repo_activity_daily (repo_id, activity_date, commit_count, additions, deletions, files_changed) VALUES (?1, ?2, ?3, ?4, ?5, ?6)"#, params![repo_id, activity_date, commit_count, additions, deletions, files_changed])?;
    Ok(())
}
...
pub fn get_repo_activity_daily(
    ...
    let mut sql = "SELECT id, repo_id, activity_date, commit_count, additions, deletions, files_changed FROM repo_activity_daily WHERE repo_id = ?1".to_string();

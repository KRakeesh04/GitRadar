...
import {
  useContributors,
  useRepositoryById,
  useRepositoryLanguagesStats,
  useToggleRepositoryStarred,
    ...
    >
      <div
        className={cn(
          ...
          isMore
            ? 'border-dashed border-muted-foreground bg-muted text-muted-foreground'
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between gap-4">
          <div>
            <h2 id="contributors-dialog-title" className="text-lg font-semibold">
              All contributors
            </h2>
            <p className="text-sm text-muted-foreground">
              Ordered by contribution to this repository
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="min-h-0 space-y-2 overflow-y-auto pr-1">
          {contributors.map(contributor => (
            <ContributorDetails
              key={`${contributor.email}-${contributor.id}`}
              contributor={contributor}
            />
          ))}
        </div>
...
export function RepositoryMetadataBar({ repoId }: { repoId: string }) {
  ...
  const languagesDetails = useMemo(
    () => getLanguageDetails(repoLanguagesStats),
    [repoLanguagesStats]
  );
  ...
  const visibleContributors =
    sortedContributors.length > 4 ? sortedContributors.slice(0, 3) : sortedContributors;
    ...
    <div className="mx-10 flex flex-wrap items-center gap-10">
      <div className="flex min-w-[18rem] flex-1 flex-col items-start gap-2">
        <div className="flex w-full items-center gap-2">
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-xs ${repoInfo?.isDirty ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}
          >
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="flex items-center text-sm">
            ...
            {formatUpdatedAt(lastCommitDate.toString())}
          </span>
        </div>
      </div>
        ...
        <div className="flex max-w-120 flex-wrap gap-2">
          {languagesDetails.map(language => (
            <div key={language.name} className="flex items-center gap-2">
              <div
                className={`h-2 w-2 rounded-full ${languageColors[language.name] ?? languageColors.Other}`}
              />
      ...
      <div className="flex min-w-50 flex-1 flex-col items-start gap-2">
        ...
        <div className="flex gap-2">
          ...
          {sortedContributors.length > 4 && (
            <ContributorAvatar onClick={() => setShowAllContributors(true)} />
          )}
        </div>
      </div>
